...
import GHC.Int (eqInt32, eqInt64)
...
exprToString (Add e1 e2 e3) = "(" ++ exprToString e1 ++ ")+(" ++ exprToString e2 ++ ")+(" ++ exprToString e3 ++ ")"
exprToString (Tangent e) = "tan(pi*(" ++ exprToString e ++ "))"
...
eval x y e =
  case e of
    ...
    Thresh e1 e2 e3 e4 -> compare e1 e2 e3 e4
      where
        compare :: Expr -> Expr -> Expr -> Expr -> Double
        compare e1 e2 e3 e4
          | eval x y e1 < eval x y e2 = eval x y e3
          | otherwise = eval x y e4
    Add e1 e2 e3 -> eval x y e1 + eval x y e2 + eval x y e3
    Tangent e1 -> tan (pi * eval x y e1)
...
build d
  ...
  | r == 2 = Sine (Average (build (d - 1)) (build (d - 1)))
  | r == 3 = Cosine (Times (build (d - 1)) (build (d - 1)))
  | r == 4 = Sine (Add (build (d - 1)) (build (d - 1)) (build (d - 1)))
  | r == 5 = Sine (Tangent (build (d - 1)))
